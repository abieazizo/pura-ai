 ...
 *     EXPO_PUBLIC token) via a single named operation `readSkin` (handled by
 *     server/lib/handlers.ts → OpenAIClient.readSkin). Prompt + schema are the
 *     SAME on both sides — the server imports them from the dependency-free
 *     `./skinReadPrompt` module (this file can't be imported by Node: aiGateway
 *     → expo-constants). The image is sent at detail:"high", strict structured
 *     outputs enforced server-side.
 *   • The resolved read is MEMOISED by photoUri (computed ONCE): screen 1
 *     (First-Finding) and screen 2 (Your Skin) read the SAME canonical result
 *     with no second API call. Failures aren't cached, so a retry re-fetches.
 *
 * Offline/preview verification uses the exported FIXTURES directly (see the dev
 * harness), so the real path stays real.
 */
...
// ===========================================================================
// THE GPT-4V SYSTEM PROMPT + STRICT SCHEMA — the single source of truth lives in
// the dependency-free `./skinReadPrompt` module so the SERVER (plain Node) can
// import it too. (THIS file pulls in aiGateway → expo-constants, which a Node
// import can't load, so the server must NOT import the prompt from here.)
// Re-exported so every existing `@/api/skinRead` importer is unchanged.
// ===========================================================================

export { SKIN_READ_SYSTEM_PROMPT, SKIN_READ_JSON_SCHEMA } from './skinReadPrompt';
...
//
// MEMOISED by photoUri so the read is computed ONCE and shared by screen 1
// (First-Finding) and screen 2 (Your Skin) — same canonical JSON, no second
// call. Only USABLE outcomes (ready / bad_photo) are cached; a service_error is
// never cached, so an in-place retry re-fetches. Tiny bounded cache.
// ===========================================================================

const _readCache = new Map<string, SkinReadOutcome>();
const READ_CACHE_MAX = 4;

function cacheRead(photoUri: string, outcome: SkinReadOutcome): SkinReadOutcome {
  if (outcome.status === 'ready' || outcome.status === 'bad_photo') {
    _readCache.set(photoUri, outcome);
    if (_readCache.size > READ_CACHE_MAX) {
...
/** Drop a memoised read (e.g. before a deliberate re-scan of the same uri). */
export function invalidateSkinRead(photoUri: string): void {
  _readCache.delete(photoUri);
}
...
export async function readSkinFromPhoto(args: {
  ...
  return cacheRead(photoUri, outcomeFromRead(read));
}
